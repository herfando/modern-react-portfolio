...
// Type definition for the `ContactUI` component's props.
// This clearly specifies the data and functions it needs from its parent component.
interface ContactUIProps {
  ...
  // This prop handles changes for both text inputs and textareas, ensuring controlled components.
  handleInputChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void;
  // This prop handles the form's submission event.
  handleSubmit: (e: React.FormEvent<HTMLFormElement>) => void;
}

// A dedicated, reusable component for a single form input field.
// This pattern reduces code repetition and makes the form easier to read.
const FormInput: React.FC<{ id: string; label: string; type: string; placeholder: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; }> = ({ id, label, type, placeholder, value, onChange }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <input
      ...
      // The `value` prop makes this a controlled component, with its state managed by the parent.
      value={value}
      onChange={onChange}
    />
  </div>
...
// A dedicated, reusable component for a single checkbox.
// This also makes the form clearer and easier to manage.
const FormCheckbox: React.FC<{ id: string; label: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; checked: boolean; }> = ({ id, label, value, onChange, checked }) => (
  <label className="flex gap-3 items-center">
    <input
      ...
      value={value}
      onChange={onChange}
      // The `checked` prop is a boolean that determines if the checkbox is selected,
      // and it's also controlled by the parent's state.
      checked={checked}
    />
    <span>{label}</span>
  </label>
...
// The main `ContactUI` component, responsible for rendering the entire contact form.
// It receives form data and handlers as props, separating its presentation from the logic.
    ...
    <form onSubmit={handleSubmit} className="md:w-[720px] mx-auto w-full">
      <fieldset className="flex flex-col gap-10">
        <div className="flex flex-col gap-5">
          {/* Using the reusable `FormInput` component for name and email fields. */}
          <FormInput
            id="name"
            label="Name"
            type="text"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-[37px] max-[393px]:flex-col max-[393px]:gap-[0px]">
              <div className="basis-[232px] flex flex-col gap-4 max-[393px]:basis-[0px]">
                {/* Using the reusable `FormCheckbox` component for services. */}
