...
const [isOpen, setIsOpen] = useState(false);
  ...
  <nav className=" w-full h-21 bg-white flex justify-between md:justify-around items-center px-4">
    ...
    <div className="flex items-center gap-2">
        <img src="./public/mylogo.png" alt="logo" />
    ...
    <button onClick={()=> setIsOpen(!isOpen)}className="text-black text-3xl md:hidden focus:outline-none z-50">
        {isOpen ? '✖' : '☰'}
    </button>
    {/* Display Hambuerger Onclick */}
    <div className={`translate-x-10 gap-5 fixed top-0 left-0 w-full h-[550px] bg-white text-black transform transition-transform duration-300 ease-in-out z-[-1] ${
        ...
        <div className="flex w-full justify-center">
            <button className="bg-[#FF623E] w-[361px] md:w-[197px] h-[44px] rounded-2xl font-bold text-white">Let's Talk</button>
        </div>    
    </div>
  </nav>
  )
